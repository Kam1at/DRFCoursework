...
class HabitTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.user = User(email='sdakhj@mail.ru', telegram_id=settings.TELEGRAM_ID, number='8-800-555-35-35')
        self.user.set_password('1234')
        self.user.save()

        response = self.client.post('/users/api/token/', {"email": 'sdakhj@mail.ru',
                                               "password": "1234"})
